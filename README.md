...
- **.md 단일 파일 이력** — 저장(`archive`/`export`)된 .md 끝에 **초소형 복원 힌트**(`<!-- A2 … -->` 한 줄, base64)만 박는다. 발언 원문은 본문에 이미 있으므로 중복 저장하지 않고, 힌트엔 형식·persona 색·**발언별 길이 앵커**만 담겨 블록 크기가 토론 길이가 아니라 (에이전트+발언) 수에만 비례한다. '불러오기' 는 본문을 파싱하되 발언에 `##`/`###` 가 섞여도 길이 앵커로 경계를 되찾아 복원한다 (옛 `AGORA-STATE-V1` 전체상태 블록 파일도 그대로 완벽 복원)
